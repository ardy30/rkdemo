...
public class DeviceManager {
     ...
     * @return
     */
    public List<MountEntity> getAllMountedDevices() {
        List<MountEntity> devices = new ArrayList<>();
        File f = Environment.getHostStorageDirectory();
        File[] mountedDevice = f.listFiles();
        if (mountedDevice != null && mountedDevice.length > 0) {
            for (File file :
                    mountedDevice) {
                MountEntity mountEntity = new MountEntity();
                if (file.getName().contains("USB") && !file.getName().contains("SATA")) {
                    mountEntity.type = MountType.USB.getType();
                } else if (file.getName().contains("SATA")) {
                    mountEntity.type = MountType.SATA.getType();
                }
                mountEntity.path = file.getAbsolutePath();
                mountEntity.totalSpace = file.getTotalSpace();
                mountEntity.usageSpace = file.getFreeSpace();
                mountEntity.usageSpace = mountEntity.totalSpace - mountEntity.usageSpace;
                devices.add(mountEntity);
            }
        }
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            ...
            if (file.getName().contains("sd")) {
                mountEntity.type = MountType.SDCARD.getType();
            }
            mountEntity.path = file.getAbsolutePath();
            mountEntity.totalSpace = file.getTotalSpace();
            mountEntity.usageSpace = file.getFreeSpace();
